...
class ConcentricCirclesDetector:
    ...
    def find_concentric_circles(self):
        for i, (x1, y1, r1) in enumerate(self.circles):
            for j, (x2, y2, r2) in enumerate(self.circles):
                if i >= j:
                    continue

                if abs(x1 - x2) < 5 and abs(y1 - y2) < 5 and abs(r1 - r2) > 5:
                    self.concentric_circles.append(((x1, y1, r1), (x2, y2, r2)))

    def draw_and_save_result(self, output_path: str):
        for (x1, y1, r1), (x2, y2, r2) in self.concentric_circles:
